...
are_there_gap!(c::C, new_ts::Int64,       time_frame) where C <: CandleType = begin
	...
	if (new_ts - c_tsend) / CANDLE_TO_MS[time_frame] > 1.05 
		...
		# +CANDLE_TO_MS["1m"]
		miss = UniversalStruct.load(C, :validation, c.exchange, c.market, c.is_futures, c.candle_type, c.candle_value, cld(c_tsend,1000), floor_ts(floor(Int,(new_ts-1)/1000),60))
